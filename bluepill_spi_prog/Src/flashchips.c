...
//		.tested		= TEST_OK_PREWU,
...

	{
		...
		.unlock		= spi_disable_blockprotect,
		.write		= spi_chip_write_af, /* AAI supported, but opcode is 0xAF */
		.read		= spi_chip_read,
		.erase    = spi_erase_bulk,
		.voltage	= {3000, 3600},
	},
...
//		.probe_timing	= TIMING_ZERO,
		...
		.unlock		= spi_disable_blockprotect,
		.write		= spi_chip_write_ad,  //SPI_WRITE_AAI
		.read		= spi_chip_read,
		.erase    = spi_erase_bulk,		
		.voltage	= {2700, 3600},
	},
...
//		.probe_timing	= TIMING_ZERO,
		...
		.unlock		= spi_disable_blockprotect,
		.write		= spi_chip_write_ad,  //SPI_WRITE_AAI  spi_chip_write_1,
		.read		= spi_chip_read,
		.erase    = spi_erase_bulk,		
		.voltage	= {2700, 3600},
	},
		...
		{
			{
				...
				.block_erase = SPI_BLOCK_ERASE_60, /* Use  0x60 */
			},
		},
...
//	},
	...
	{
		.vendor		= "Generic",
		.name		= "unknown SPI chip or wrong connections",// connections",
		.bustype	= BUS_SPI,
		.manufacture_id	= GENERIC_MANUF_ID,
		.model_id	= GENERIC_DEVICE_ID,
		.total_size	= 0,
...
unsigned int flashchips_in_base(void){
	uint16_t i=0;
	while(flashchips[i].total_size != 0)
	{
		i++;
	}
	return (i-1);
